...
def menu():
    a = Context()
    Observer.attach('add', Logger.Add)
    Observer.attach('delete', Logger.Delete)
    Observer.attach('method', Logger.Method)
...
def generate(a):
    ...
    Event.update('add', former, pos, a.GetList())


def delete(a):
    if Validation.validateContext(a):
        pos = int(input('Enter position: '))
        starting = a.GetList().copy()
        a.GetList().pop(pos)
        Event.update('delete', starting, pos, a.GetList())
        print('Item deleted!')
...
def deleteinrange(a):
    if Validation.validateContext(a):
        ...
        Event.update('delete', starting, [start, end], a.GetList())
...
def method(a):
    starting = a.GetList().copy()
    result = a.GetList().indexsort().copy()
    Event.update('method', starting, result)
